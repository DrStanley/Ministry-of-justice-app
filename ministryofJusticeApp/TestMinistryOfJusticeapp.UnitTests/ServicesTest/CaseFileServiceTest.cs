...
namespace TestMinistryOfJusticeapp.UnitTests.ServicesTest
{
    ...
    public class CaseFileServiceTest
    {
        ...
        {
            //Arrange
            _mapper.Setup(mf => mf.Map<UploadFileViewModels, File>(_fileView)).Callback(() => throw new Exception());
            _caseFileRepo.Setup(fs => fs.UploadFile(_filed, path, _file.Object))
                .Callback(() => throw new Exception());


            //Assert
            var ex = Assert.Throws<Exception>(() =>
                _caseFile.UploadCaseFile(_fileView, It.IsAny<string>(), _file.Object));
        ...
        public void NewTest_For_DeleteCaseFile(int id)
        {
            ...
            _unitOfWorkMock.Verify(x => x.CaseFileRepo.Save());
        }
